...
from src.python.modules.bioasqbiobert import modeling, optimization, tokenization
...
def create_doc_tokens(paragraph_text):
    doc_tokens = []
...
def read_squad_example_from_dict(input):
    context = input['context']
    qa_id = input['id']
    question = input["question"]
    doc_tokens = create_doc_tokens(context)
    return SquadExample(qas_id=qa_id,
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
                      ...
                      output_nbest_file, output_null_log_odds_file, to_dump_to_file=True):
    ...
    if to_dump_to_file:
        ...
        return [
            {"id": a, "text": b[0].get('text', ''), "probability": b[0].get('probability', 1.0)} if len(b) != 0 else {
                "id": a, "text": '', "probability": 1.0} for a, b in all_nbest_json.items()]
...
#
#     write_predictions(eval_examples, eval_features, all_results, FLAGS.n_best_size, FLAGS.max_answer_length,
